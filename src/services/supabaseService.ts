...
import { isOldPharmacyTestData } from '../utils/cleanOldPharmacyData';
import { 
  MOCK_PRESCRIPTIONS, 
  MOCK_NURSE_SHIFTS, 
  MOCK_THEATRES,
...
function cleanInvoiceForPostgres(inv: any) {
  ...
  if (!cleaned.invoice_number) {
    const typeUpper = String(cleaned.type || cleaned.invoice_type || '').toUpperCase();
    if (typeUpper === 'PHARMACY') {
      cleaned.invoice_number = `INV-POS-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    } else if (typeUpper === 'OPD') {
      cleaned.invoice_number = `INV-OPD-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    } else if (typeUpper === 'MATERNITY') {
      cleaned.invoice_number = `INV-MAT-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    } else if (typeUpper === 'LAB') {
      cleaned.invoice_number = `INV-LAB-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    } else if (typeUpper === 'INDEPENDENT') {
      cleaned.invoice_number = `INV-IND-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    } else {
      cleaned.invoice_number = `INV-IND-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    }
  }
...
function mapInvoiceFromPostgres(inv: any) {
  ...
  if (!iType) {
    ...
    if (numUpper.startsWith('INV-OPD') || numUpper.startsWith('INV-REG')) {
      ...
      iType = 'Pharmacy';
    } else if (numUpper.startsWith('INV-IND')) {
      iType = 'Independent';
    } else if (numUpper.startsWith('INV-LAB')) {
      iType = 'Lab';
    } else {
...
const rawSupabaseService = {
  ...
  getInvoices: async () => {
    try {
      ...
      const dbInvoices = data ? data.map((inv: any) => {
        ...
        return !isDummyPatient(pat) && !isOldPharmacyTestData(inv);
      }) : [];
      ...
      // Merge: any local/mock invoice that is not present in DB, excluding old pharmacy test data
      const offlineInvoices = localInvoices.filter((li: any) => {
        if (!li) return false;
        if (isOldPharmacyTestData(li)) return false;
  ...
  updateInvoice: async (id: string, invoice: any, items?: any[]) => {
    try {
      if (id.startsWith('virtual-inv-opd-')) {
        ...
        let paymentStatus = invoice.status || invoice.payment_status || 'Paid';
        if (paymentStatus === 'Settled') paymentStatus = 'Paid';
        
        const aptUpdates: any = {
          payment_status: paymentStatus,
          paymentStatus: paymentStatus,
          discount_amount: invoice.discount_amount ?? invoice.discountAmount ?? 0,
          discountAmount: invoice.discount_amount ?? invoice.discountAmount ?? 0,
          refund_given_by: invoice.refund_given_by || invoice.refundGivenBy || null,
          refundGivenBy: invoice.refund_given_by || invoice.refundGivenBy || null,
          paymentRemarks: invoice.payment_remarks || invoice.remarks || ''
        };
        
        const updatedApt = await supabaseService.updateAppointment(aptId, aptUpdates);
        
        // Update local storage appointments fallback
        const appointments = storage.get(STORAGE_KEYS.APPOINTMENTS, []);
        const updatedApts = appointments.map((a: any) => {
          if (a.id === aptId) {
            return {
              ...a,
              payment_status: paymentStatus,
        ...
        const baseFee = Number(updatedApt?.fee || 500);
        const discount = Number(updatedApt?.discount_amount || updatedApt?.discountAmount || 0);
        const feeToCollect = Math.max(0, baseFee - discount);
        
        return {
          id: id,
          patient_id: updatedApt?.patient_id || updatedApt?.patientId,
          invoice_number: `INV-OPD-V-${aptId}`,
          status: paymentStatus,
          payment_status: paymentStatus,
          total_amount: baseFee,
          discount_amount: discount,
          payable_amount: feeToCollect,
          paid_amount: paymentStatus === 'Refunded' ? 0 : feeToCollect,
          payment_method: invoice.payment_method || 'Cash',
          type: 'OPD',
          created_at: updatedApt?.created_at || new Date().toISOString()
        };
      }
  ...
  createLabTest: async (labTest: any) => {
    try {
      const payload = {
        id: labTest.id && isUuid(labTest.id) ? labTest.id : toDeterministicUuid(labTest.id || 'lt-' + Date.now()),
        name: labTest.name,
  ...
  getBedRates: async () => {
    try {
      ...
      console.error('Error fetching bed rates:', error.message);
      return null;
    }
  },

  createBedRate: async (bedRate: any) => {
    try {
      const payload = {
        type: bedRate.type,
        rate: parseFloat(bedRate.rate) || 0
      };
      const { data, error } = await supabase
        .from('bed_rates')
        .insert([payload])
      ...
      console.error('Error creating bed rate:', error.message);
      return null;
    }
  },

  updateBedRate: async (type: string, rate: number) => {
    try {
      const { data, error } = await supabase
        .from('bed_rates')
        .update({ rate })
      ...
      console.error('Error updating bed rate:', error.message);
      return null;
    }
  },

  deleteBedRate: async (type: string) => {
    try {
      const { error } = await supabase
        .from('bed_rates')
        .delete()
      ...
      console.error('Error deleting bed rate:', error.message);
      return false;
    }
  },
  ...
  getOtRates: async () => {
    try {
      ...
      console.error('Error fetching ot rates:', error.message);
      return null;
    }
  },

  createOtRate: async (otRate: any) => {
    try {
      const payload = {
        type: otRate.type,
        rate: parseFloat(otRate.rate) || 0
      };
      const { data, error } = await supabase
        .from('ot_rates')
        .insert([payload])
      ...
      console.error('Error creating ot rate:', error.message);
      return null;
    }
  },

  updateOtRate: async (type: string, rate: number) => {
    try {
      const { data, error } = await supabase
        .from('ot_rates')
        .update({ rate })
      ...
      console.error('Error updating ot rate:', error.message);
      return null;
    }
  },

  deleteOtRate: async (type: string) => {
    try {
      const { error } = await supabase
        .from('ot_rates')
        .delete()
      ...
      console.error('Error deleting ot rate:', error.message);
      return false;
    }
  },

  // Material Rates
  getMaterialRates: async () => {
    try {
      const { data, error } = await supabase
        .from('material_rates')
        .select('*')
      ...
      console.error('Error fetching material rates:', error.message);
      return null;
    }
  },

  createMaterialRate: async (materialRate: any) => {
    try {
      const payload = {
        name: materialRate.name,
        category: materialRate.category,
        price: parseFloat(materialRate.price) || 0
      };
      const { data, error } = await supabase
        .from('material_rates')
        .insert([payload])
      ...
      console.error('Error creating material rate:', error.message);
      return null;
    }
  },

  updateMaterialRate: async (name: string, updates: any) => {
    try {
      const payload: any = {};
      if (updates.category !== undefined) payload.category = updates.category;
      if (updates.price !== undefined) payload.price = parseFloat(updates.price) || 0;
      ...
      console.error('Error updating material rate:', error.message);
      return null;
    }
  },

  deleteMaterialRate: async (name: string) => {
    try {
      const { error } = await supabase
        .from('material_rates')
        .delete()
      ...
      console.error('Error deleting material rate:', error.message);
      return false;
    }
  },
  ...
  getOpdCharges: async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*');
      if (error) throw error;
      const chargesObj: any = { reg: 200, appt: 200, consult: 500 };
      if (data && data.length > 0) {
        data.forEach((row: any) => {
          chargesObj[row.key] = parseFloat(row.value) || 0;
        });
      }
      return chargesObj;
    } catch (error: any) {
      console.error('Error fetching OPD charges:', error.message);
      return null;
    }
  },

  updateOpdCharges: async (charges: { reg: number; appt: number; consult: number }) => {
    try {
      const rows = [
        { key: 'reg', value: charges.reg },
        { key: 'appt', value: charges.appt },
        { key: 'consult', value: charges.consult }
      ];
      const { data, error } = await supabase
        .from('opd_charges')
        .upsert(rows)
      ...
      console.error('Error updating OPD charges:', error.message);
      return null;
    }
  },
...
const cacheConfig: Record<string, { storageKey: string; defaultVal: any }> = {
  ...
  getBedRates: { storageKey: STORAGE_KEYS.BED_RATES, defaultVal: MOCK_BED_RATES },
