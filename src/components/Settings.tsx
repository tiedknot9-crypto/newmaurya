...
export default function Settings({ currentUser, onUserUpdate, onHospitalUpdate }: { currentUser?: any, onUserUpdate?: (user: any) => void, onHospitalUpdate?: (info: any) => void }) {
  ...
  useEffect(() => {
    const loadAllSettings = async () => {
      try {
        ...
        if (supabaseService.getBedRates) {
          const dbBedRates = await supabaseService.getBedRates();
          if (dbBedRates && dbBedRates.length > 0) {
            setBedRates(dbBedRates);
            storage.set(STORAGE_KEYS.BED_RATES, dbBedRates);
          }
        }

        if (supabaseService.getOtRates) {
          const dbOtRates = await supabaseService.getOtRates();
          if (dbOtRates && dbOtRates.length > 0) {
            setOtRates(dbOtRates);
            storage.set(STORAGE_KEYS.OT_RATES, dbOtRates);
          }
        }

        if (supabaseService.getLabTests) {
          const dbLabRates = await supabaseService.getLabTests();
          if (dbLabRates && dbLabRates.length > 0) {
            setLabRates(dbLabRates);
            storage.set(STORAGE_KEYS.LAB_RATES, dbLabRates);
          }
        }

        if (supabaseService.getMaterialRates) {
          const dbMaterialRates = await supabaseService.getMaterialRates();
          if (dbMaterialRates && dbMaterialRates.length > 0) {
            setMaterialRates(dbMaterialRates);
            storage.set(STORAGE_KEYS.MATERIAL_RATES, dbMaterialRates);
          }
        }

        if (supabaseService.getOpdCharges) {
          const dbOpdCharges = await supabaseService.getOpdCharges();
          if (dbOpdCharges) {
            setOpdCharges(dbOpdCharges);
            storage.set(STORAGE_KEYS.OPD_CHARGES, dbOpdCharges);
          }
        }
      } catch (err) {
  ...
  // Rates Editing States
  const [editingBedRate, setEditingBedRate] = useState<{ type: string; rate: string } | null>(null);
  const [editingBedRateOriginalType, setEditingBedRateOriginalType] = useState<string>('');

  const [editingOtRate, setEditingOtRate] = useState<{ type: string; rate: string } | null>(null);
  const [editingOtRateOriginalType, setEditingOtRateOriginalType] = useState<string>('');

  const [editingLabRate, setEditingLabRate] = useState<{ id: string; name: string; category: 'Pathology' | 'Radiology'; price: string } | null>(null);
  const [editingLabRateId, setEditingLabRateId] = useState<string>('');

  const [editingMaterialRate, setEditingMaterialRate] = useState<{ name: string; category: 'Disposable' | 'Material'; price: string } | null>(null);
  const [editingMaterialRateOriginalName, setEditingMaterialRateOriginalName] = useState<string>('');
  ...
  const handleAddBedRate = async () => {
    if (!newBedRate.type || !newBedRate.rate) return;
    const rateVal = parseInt(newBedRate.rate);
    const newRateObj = { type: newBedRate.type, rate: rateVal };

    setBedRates([...bedRates, newRateObj]);
    setNewBedRate({ type: '', rate: '' });
    toast.success('Bed rate added');

    if (supabaseService.createBedRate) {
      await supabaseService.createBedRate(newRateObj);
    }
  };

  const handleUpdateBedRate = async () => {
    if (!editingBedRate || !editingBedRate.type || !editingBedRate.rate) return;
    const rateVal = parseInt(editingBedRate.rate);
    const updated = bedRates.map(r => r.type === editingBedRateOriginalType ? { type: editingBedRate.type, rate: rateVal } : r);
    setBedRates(updated);
    setEditingBedRate(null);
    setEditingBedRateOriginalType('');
    toast.success('Bed rate updated');

    if (supabaseService.updateBedRate) {
      await supabaseService.updateBedRate(editingBedRateOriginalType, rateVal);
    }
  };
  ...
  const handleAddOtRate = async () => {
    if (!newOtRate.type || !newOtRate.rate) return;
    const rateVal = parseInt(newOtRate.rate);
    const newRateObj = { type: newOtRate.type, rate: rateVal };

    setOtRates([...otRates, newRateObj]);
    setNewOtRate({ type: '', rate: '' });
    toast.success('OT rate added');

    if (supabaseService.createOtRate) {
      await supabaseService.createOtRate(newRateObj);
    }
  };

  const handleUpdateOtRate = async () => {
    if (!editingOtRate || !editingOtRate.type || !editingOtRate.rate) return;
    const rateVal = parseInt(editingOtRate.rate);
    const updated = otRates.map(r => r.type === editingOtRateOriginalType ? { type: editingOtRate.type, rate: rateVal } : r);
    setOtRates(updated);
    setEditingOtRate(null);
    setEditingOtRateOriginalType('');
    toast.success('OT charge updated');

    if (supabaseService.updateOtRate) {
      await supabaseService.updateOtRate(editingOtRateOriginalType, rateVal);
    }
  };
  ...
  const handleAddLabRate = async () => {
    if (!newLabRate.name || !newLabRate.price) return;
    const priceVal = parseInt(newLabRate.price);
    const newRateObj = { id: `lt-${Date.now()}`, name: newLabRate.name, category: newLabRate.category, price: priceVal };

    setLabRates([...labRates, newRateObj]);
    setNewLabRate({ ...newLabRate, name: '', price: '' });
    toast.success('Lab/Radiology rate added');

    if (supabaseService.createLabTest) {
      await supabaseService.createLabTest(newRateObj);
    }
  };

  const handleUpdateLabRate = async () => {
    if (!editingLabRate || !editingLabRate.name || !editingLabRate.price) return;
    const priceVal = parseInt(editingLabRate.price);
    const updated = labRates.map(r => (r.id || r.name) === editingLabRateId ? { ...r, name: editingLabRate.name, category: editingLabRate.category, price: priceVal } : r);
    setLabRates(updated);
    setEditingLabRate(null);
    setEditingLabRateId('');
    toast.success('Lab/Radiology rate updated');

    if (supabaseService.updateLabTest) {
      await supabaseService.updateLabTest(editingLabRateId, {
        name: editingLabRate.name,
        category: editingLabRate.category,
        price: priceVal
      });
    }
  };
  ...
  const handleAddMaterialRate = async () => {
    if (!newMaterialRate.name || !newMaterialRate.price) return;
    const priceVal = parseInt(newMaterialRate.price);
    const newRateObj = { name: newMaterialRate.name, price: priceVal, category: newMaterialRate.category };

    setMaterialRates([...materialRates, newRateObj]);
    setNewMaterialRate({ ...newMaterialRate, name: '', price: '' });
    toast.success('Material rate added');

    if (supabaseService.createMaterialRate) {
      await supabaseService.createMaterialRate(newRateObj);
    }
  };

  const handleUpdateMaterialRate = async () => {
    if (!editingMaterialRate || !editingMaterialRate.name || !editingMaterialRate.price) return;
    const priceVal = parseInt(editingMaterialRate.price);
    const updated = materialRates.map(r => r.name === editingMaterialRateOriginalName ? { ...r, name: editingMaterialRate.name, category: editingMaterialRate.category, price: priceVal } : r);
    setMaterialRates(updated);
    setEditingMaterialRate(null);
    setEditingMaterialRateOriginalName('');
    toast.success('Material rate updated');

    if (supabaseService.updateMaterialRate) {
      await supabaseService.updateMaterialRate(editingMaterialRateOriginalName, {
        category: editingMaterialRate.category,
        price: priceVal
      });
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50/50 min-h-screen">
      ...
      <Tabs defaultValue={isAccountant ? "rates" : "hospital"} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="border-none shadow-sm">
              ...
              <CardContent className="space-y-4">
                {!isAccountant && (
                  editingBedRate ? (
                    <div className="grid grid-cols-2 gap-2 bg-amber-50/50 p-2 rounded border border-amber-100">
                      <Input 
                        placeholder="Bed Type (e.g. Deluxe)" 
                        value={editingBedRate.type} 
                        onChange={(e) => setEditingBedRate({...editingBedRate, type: e.target.value})} 
                      />
                      <div className="flex gap-2">
                        <Input 
                          type="number" 
                          placeholder="Rate / Day" 
                          value={editingBedRate.rate} 
                          onChange={(e) => setEditingBedRate({...editingBedRate, rate: e.target.value})} 
                        />
                        <Button className="bg-amber-600 text-white hover:bg-amber-700" onClick={handleUpdateBedRate}><Save className="w-4 h-4" /></Button>
                        <Button variant="outline" size="icon" onClick={() => { setEditingBedRate(null); setEditingBedRateOriginalType(''); }}><Plus className="w-4 h-4 rotate-45 text-rose-500" /></Button>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="space-y-2">
                  {bedRates.map((rate) => (
                    <div key={rate.type} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-medical-blue mr-2">₹{rate.rate} / Day</span>
                        {!isAccountant && (
                          <div className="flex gap-1">
                            <Button 
                              variant="ghost" 
                              size="icon" 
                              className="h-8 w-8 text-medical-blue hover:text-medical-blue/80" 
                              onClick={() => {
                                setEditingBedRate({ type: rate.type, rate: rate.rate.toString() });
                                setEditingBedRateOriginalType(rate.type);
                                toast.info('Editing bed rate: ' + rate.type);
                              }}
                            >
                              <Edit className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 hover:text-rose-600" onClick={() => handleDeleteBedRate(rate.type)}>
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        )}
                  ...
                  editingOtRate ? (
                    <div className="grid grid-cols-2 gap-2 bg-amber-50/50 p-2 rounded border border-amber-100">
                      <Input 
                        ...
                        value={editingOtRate.type} 
                        onChange={(e) => setEditingOtRate({...editingOtRate, type: e.target.value})} 
                      />
                      <div className="flex gap-2">
                        <Input 
                          type="number" 
                          placeholder="Fixed Charge" 
                          value={editingOtRate.rate} 
                          onChange={(e) => setEditingOtRate({...editingOtRate, rate: e.target.value})} 
                        />
                        <Button className="bg-amber-600 text-white hover:bg-amber-700" onClick={handleUpdateOtRate}><Save className="w-4 h-4" /></Button>
                        <Button variant="outline" size="icon" onClick={() => { setEditingOtRate(null); setEditingOtRateOriginalType(''); }}><Plus className="w-4 h-4 rotate-45 text-rose-500" /></Button>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-2">
                ...
                <div className="space-y-2">
                  {otRates.map((rate) => (
                    <div key={rate.type} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-medical-blue mr-2">₹{rate.rate}</span>
                        {!isAccountant && (
                          <div className="flex gap-1">
                            <Button 
                              variant="ghost" 
                              ...
                              className="h-8 w-8 text-medical-blue hover:text-medical-blue/80" 
                              onClick={() => {
                                setEditingOtRate({ type: rate.type, rate: rate.rate.toString() });
                                setEditingOtRateOriginalType(rate.type);
                                toast.info('Editing OT charge: ' + rate.type);
                              }}
                            >
                              <Edit className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 hover:text-rose-600" onClick={() => handleDeleteOtRate(rate.type)}>
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        )}
                ...
                <div className="pt-2 flex justify-end">
                  <Button 
                    ...
                    onClick={async () => {
                      ...
                      if (supabaseService.updateOpdCharges) {
                        await supabaseService.updateOpdCharges(opdCharges);
                      }
                    }}
                  >
                    Save Charges
            ...
            <Card className="border-none shadow-sm lg:col-span-2">
              ...
              <CardContent className="space-y-4">
                {!isAccountant && (
                  editingLabRate ? (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-2 bg-amber-50/50 p-2 rounded border border-amber-100">
                      <Input 
                        placeholder="Test Name" 
                        value={editingLabRate.name} 
                        onChange={(e) => setEditingLabRate({...editingLabRate, name: e.target.value})} 
                        className="md:col-span-2"
                      />
                      <Select value={editingLabRate.category} onValueChange={(v: any) => setEditingLabRate({...editingLabRate, category: v})}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <div className="flex gap-2">
                        <Input 
                          type="number" 
                          placeholder="Price" 
                          value={editingLabRate.price} 
                          onChange={(e) => setEditingLabRate({...editingLabRate, price: e.target.value})} 
                        />
                        <Button className="bg-amber-600 text-white hover:bg-amber-700" onClick={handleUpdateLabRate}><Save className="w-4 h-4" /></Button>
                        <Button variant="outline" size="icon" onClick={() => { setEditingLabRate(null); setEditingLabRateId(''); }}><Plus className="w-4 h-4 rotate-45 text-rose-500" /></Button>
                      </div>
                    </div>
                  ) : (
                  ...
                  <div className="space-y-2">
                    {labRates.map((rate: any) => (
                      <div key={rate.id || rate.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold text-medical-blue mr-2">₹{rate.price}</span>
                          {!isAccountant && (
                            <div className="flex gap-1">
                              <Button 
                                variant="ghost" 
                                ...
                                className="h-8 w-8 text-medical-blue hover:text-medical-blue/80" 
                                onClick={() => {
                                  setEditingLabRate({ id: rate.id || rate.name, name: rate.name, category: rate.category, price: rate.price.toString() });
                                  setEditingLabRateId(rate.id || rate.name);
                                  toast.info('Editing lab/radiology rate: ' + rate.name);
                                }}
                              >
                                <Edit className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 hover:text-rose-600" onClick={() => handleDeleteLabRate(rate)}>
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </div>
                          )}
                  ...
                  editingMaterialRate ? (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-2 bg-amber-50/50 p-2 rounded border border-amber-100">
                      <Input 
                        ...
                        value={editingMaterialRate.name} 
                        onChange={(e) => setEditingMaterialRate({...editingMaterialRate, name: e.target.value})} 
                        className="md:col-span-2"
                      />
                      <Select value={editingMaterialRate.category} onValueChange={(v: any) => setEditingMaterialRate({...editingMaterialRate, category: v})}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <div className="flex gap-2">
                        <Input 
                          type="number" 
                          placeholder="Price" 
                          value={editingMaterialRate.price} 
                          onChange={(e) => setEditingMaterialRate({...editingMaterialRate, price: e.target.value})} 
                        />
                        <Button className="bg-amber-600 text-white hover:bg-amber-700" onClick={handleUpdateMaterialRate}><Save className="w-4 h-4" /></Button>
                        <Button variant="outline" size="icon" onClick={() => { setEditingMaterialRate(null); setEditingMaterialRateOriginalName(''); }}><Plus className="w-4 h-4 rotate-45 text-rose-500" /></Button>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                          ...
                          {!isAccountant && (
                            <div className="flex gap-1">
                              <Button 
                                ...
                                className="h-8 w-8 text-medical-blue hover:text-medical-blue/80" 
                                onClick={() => {
                                  setEditingMaterialRate({ name: rate.name, category: rate.category, price: rate.price.toString() });
                                  setEditingMaterialRateOriginalName(rate.name);
                                  toast.info('Editing material rate: ' + rate.name);
                                }}
                              >
                                <Edit className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 hover:text-rose-600" onClick={() => handleDeleteMaterialRate(rate.name)}>
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </div>
                          )}
